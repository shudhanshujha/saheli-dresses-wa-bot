...
function initBrowser(sessionId, config = {}, spinner) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        /**
         * Explicit fallback due to pptr 19 — force system Chromium in container
         */
        if (!config.executablePath || !require("fs").existsSync(config.executablePath)) {
            config.executablePath = process.env.PUPPETEER_EXECUTABLE_PATH || '/usr/bin/chromium';
        }
        ...
        const browser = (config === null || config === void 0 ? void 0 : config.browserWSEndpoint) ? yield puppeteer.connect(Object.assign({}, config)) : yield puppeteer.launch(Object.assign(Object.assign({ headless: "new", args }, config), { devtools: false })).catch(e => {
            ...
            console.error(e);
            if (e.message.includes("ENOENT") || e.message.includes("Failed to launch")) {
                config.executablePath = process.env.PUPPETEER_EXECUTABLE_PATH || '/usr/bin/chromium';
